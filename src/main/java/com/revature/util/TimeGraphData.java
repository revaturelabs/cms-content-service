 ...
 * purpose of storing data needed for the Time Graph, as seen in the Reports
 * page.
 * 
 * @author
 * @version 1.0
...
public class TimeGraphData {
	/**
	 * returnedLongs - Represents a set of timestamps. It is designed in expectation
	 * that there will be no duplicate millisecond times, which, while possible,
	 * will not be accounted for.
	 */
	private List<Long> returnedLongs;

	/**
	 * numContents - Represents the number of timestamped records that were created
	 * before the date that is being tracked from.
	 */
	...
	public int hashCode() {
		final int prime = 31;
		int result = 1;
		result = prime * result + numContents;
		result = prime * result + ((returnedLongs == null) ? 0 : returnedLongs.hashCode());
		return result;
	}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		TimeGraphData other = (TimeGraphData) obj;
		if (numContents != other.numContents)
			return false;
		if (returnedLongs == null) {
			if (other.returnedLongs != null)
				return false;
		} else if (!returnedLongs.equals(other.returnedLongs))
			return false;
		return true;
	}
